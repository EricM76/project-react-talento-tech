...
export const Dashboard = () => {
  ...
  const fulfillmentKpis = useMemo(
    () => [
      { label: 'Tiempo medio de entrega', value: '2.6 días', progress: 74 },
      { label: 'Nivel de satisfacción', value: '4.6 / 5', progress: 92 },
      { label: 'Tickets resueltos 24h', value: '87%', progress: 87 }
    ],
    []
  )
  ...
  const handleNavigate = () => setIsAddProductVisible(false)
  const handleOpenAddProduct = () => setIsAddProductVisible(true)
        ...
        <header className='admin-header'>
          ...
          <div>
            <h1 className='admin-title'>
              {isAddProductVisible ? 'Agregar nuevo producto' : 'Panel de Administración'}
            </h1>
            <p className='admin-subtitle'>
              {isAddProductVisible
                ? 'Complete el formulario para sumar un producto al catálogo'
                : 'Resumen general del desempeño de la tienda'}
            </p>
          </div>
        </header>

        {isAddProductVisible ? (
          <section className='dashboard-section add-product-container' id='agregar-producto'>
